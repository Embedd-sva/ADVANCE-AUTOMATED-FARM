...
uint8_t SpeechRecognizer::spch_recg(uint16_t *v_dat, uint32_t *mtch_dis)
{
    ...
    do {
        ...
        // Collect audio data
        while (frame_index < num) {
            if (i2s_rec_flag == 0) continue;
            for (i = 0; i < frame_mov; i++) {
                v_dat[frame_mov * frame_index + i] = (i2s_rec_flag == 1) ? rx_buf[i] : rx_buf[i + frame_mov];
            }
            i2s_rec_flag = 0;
            frame_index++;
        }
        ...
        noise_atap(v_dat, atap_len, &atap_arg);
        if (atap_arg.s_thl <= 50000) break; // Exit loop if calibration successful

        Serial.printf("Noise level too high, recalibrating...\n");

    } while (true);

    Serial.printf("Starting speech recognition...\n");
    ...
    while (1) {
        if (i2s_rec_flag == 0) continue;
        for (i = 0; i < frame_mov; i++) {
            v_dat[i + frame_mov] = (i2s_rec_flag == 1) ? rx_buf[i] : rx_buf[i + frame_mov];
        }
        i2s_rec_flag = 0;
    ...
    if (!voice_detected) {
        Serial.printf("VAD failed.\n");
        *mtch_dis = dis_err;
        return 0;
    }

    Serial.printf("VAD successful. Extracting MFCC...\n");

    // Extract MFCC features
    get_mfcc(&(valid_voice[0]), &ftr, &atap_arg);
    if (ftr.frm_num == 0) {
        *mtch_dis = dis_err;
        Serial.printf("MFCC extraction failed.\n");
        return 0;
    }

    Serial.printf("MFCC extraction successful.\n");
    ...
    for (ftr_addr = ftr_start_addr; ftr_addr < ftr_end_addr; ftr_addr += size_per_ftr) {
        ...
        if (ftr_mdl->save_sign == save_mask) {
            Serial.printf("Feature %d: Frame number = %d, Save mask = %d, Distance = %d\n",
                          (ftr_addr / size_per_ftr) + 1, ftr_mdl->frm_num, ftr_mdl->save_sign, cur_dis);
        }
        
        if (cur_dis < min_dis) {
            min_dis = cur_dis;
            min_comm = (ftr_addr / size_per_ftr) + 1;
        }
    }
    cycle1 = read_csr(mcycle) - cycle0;
    Serial.printf("[INFO] Recognition cycle time = 0x%08x\n", cycle1);

    // Adjust command number if necessary
    if (min_comm % 4) {
